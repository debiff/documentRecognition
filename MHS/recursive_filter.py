...
def homogeneity(region):
    if region.vertical_variance_black > 1.3 and region.vertical_variance_white >1.3:
        split_black(region, 'vertical')

    if region.horizontal_variance_black > 1.3 and region.horizontal_variance_white >1.3:
        print('non hom')
...
def split_black(region, direction):
    ...
    i_max_rle = ((i_max_black + 1) * 4) - 1 if region.vertical_rle[0] == 0 else (i_max_black * 4) + 1
    bb_list = []
    region_list = []
    count = 0
    for i in range(1, i_max_rle + 1, 2):
        if (i_max_rle == i == 1) or (i_max_rle == i + 2 == 3):
            ...
            region_left = [region.xmin, region.ymin, count, region.ymax] if direction == 'vertical' \
                else [region.xmin, region.ymin, region.xmax, count]
            region_right = [count + 1, region.ymin, region.xmax, region.ymax] if direction == 'vertical' \
                else [region.xmin, count + 1, region.xmax, region.ymax]
            ...
            region_left = [region.xmin, region.ymin, cut_left, region.ymax] if direction == 'vertical' \
                else [region.xmin, region.ymin, region.xmax, cut_left]

            count += math.floor(rle[i] / 2) + rle[i + 2] + math.ceil(rle[i + 4] / 2)
            cut_right = count

            region_center = [cut_left + 1, region.ymin, cut_right, region.ymax] if direction == 'vertical' \
                else [region.xmin, cut_left + 1, region.xmax, cut_right]

            region_right = [cut_right + 1, region.ymin, region.xmax, region.ymax] if direction == 'vertical' \
                else [region.xmin, cut_right + 1, region.xmax, region.ymax]
...
def split_white(region, direction):
    ...
    i_max_rle = ((i_max_white + 1) * 4) - 1 if region.vertical_rle[0] == 1 else (i_max_white * 4) + 1
    bb_list = []
    region_list = []
    count = 0
    for i in range(1, i_max_rle + 1, 2):
        if (i_max_rle == i == 1) or (i_max_rle == i == 3):
            ...
            count += rle[i]
            region_right = [count, region.ymin, region.xmax, region.ymax] if direction == 'vertical' \
                else [region.xmin, count, region.xmax, region.ymax]
            bb_list.extend([region_left, region_right])
        else:
            count += rle[i]
